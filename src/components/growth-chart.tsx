      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" style={{ overflow: "visible" }}>
        ...
        <defs>
          {data.map((_, i) => (
            <clipPath key={i} id={`thumb-clip-${i}`}>
              <rect x={x(i) - 24} y={y(data[i].cumulativeViews) - 47} width={48} height={36} rx={4} />
            </clipPath>
          ))}
        </defs>
        ...
        {(() => {
          ...
          for (let i = data.length - 1; i >= 0; i--) {
            ...
            const cx = x(i);
            const cy = y(data[i].cumulativeViews) - 30;
            let overlaps = false;
            for (const vi of visible) {
              const dx = Math.abs(cx - x(vi));
              const dy = Math.abs(cy - (y(data[vi].cumulativeViews) - 30));
